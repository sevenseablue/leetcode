...
class Solution:
    ...
    def ladderLength(self, beginWord, endWord, wordList):
        ...
        def constructGraph2(wordList):
            ...
            for i in range(0, len(wordList)):
                ...
                for j in range(len(word)):
                    wp, ws = word[:j], word[j + 1:]
                    for c1 in chars:
                        if c1 != word[j]:
                            onediff = wp+c1+ws
                            if onediff in wordDic:
                                graph[word].append(onediff)
                                # graph[onediff].append(word)
            return graph

        def insert(graph, word):
        ...
        graph = constructGraph2(wordList)
        insert(graph, beginWord)
        ...
        while que:
            ...
            if w1 == endWord:
                return cnt
            for wtmp in graph[w1]:
                if wtmp not in mark:
                    que.append((wtmp, cnt+1))
                    mark.add(wtmp)
        return 0
